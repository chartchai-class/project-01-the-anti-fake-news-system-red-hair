...
<template>
  <div class="flex items-center gap-2 justify-center mt-6">
    <button class="px-3 py-1 rounded bg-black text-white hover:bg-[#720000]" @click="to(page-1)" :disabled="page<=1">Prev</button>
    <span class="text-sm">Page {{ page }} / {{ totalPages }}</span>
    <button class="px-3 py-1 rounded bg-black text-white hover:bg-[#720000]" @click="to(page+1)" :disabled="page>=totalPages">Next</button>
  </div>
</template>
