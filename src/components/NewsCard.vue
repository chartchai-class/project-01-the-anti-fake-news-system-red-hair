...
<template>
  <article class="rounded-xl border bg-white overflow-hidden shadow-sm">
    <img v-if="item.image" :src="item.image" class="w-full h-48 object-cover" alt="">
    <div class="p-4">
      ...
      <div class="flex items-center gap-2 text-xs mb-2">
        <span class="px-2 py-0.5 rounded-full"
          ...
          {{ item.voteType==='fake' ? 'Fake' : 'Notfake' }}
        </span>

        <span class="text-gray-400">• {{ (item.newsDateTime instanceof Date ? item.newsDateTime : new Date(item.newsDateTime as any)).toLocaleString() }}</span>
      </div>
